...
import trabalho2.INE5633_logica_fuzzy.Controlador;
...
public class RobotControllerDirector {
	...
	/**
	 * The path of where controllers should reside with respect to the current
	 * working directory of the Khepera Simulator. The default location where
	 * controllers should be placed is in "./controllers/". For example if the
	 * simulator is running from the directory "c:\KheperaSimulator\", then the
	 * controller class files would be located in
	...
	/**
	 * Find the available controllers that can be loaded and update the hashmap if
	 * needed.
	 */
	 ...
	 * successfully. Only one instance of a particular controller can be loaded at
	 * one time. If the controller is already loaded or it failed to load, this
	...
	protected boolean startController(String controllerName) {
		...
		controller.initialize("Controlador", currentRobotState, controllerThreadWaitTime);
		controller.simStart();
		return true;
	 ...
	 * immediatly. The controller is simply informed that it is time to finish. The
